...
def _install_diariZen_pipeline_compat():
    ...
    def patched_init(self, diarizen_hub, embedding_model, config_parse=None, rttm_out_dir=None):
        ...
        if config.get("clustering", {}).get("args", {}).get("method") == "VBxClustering":
            print("[COMPAT] Converting VBxClustering to AgglomerativeClustering for pyannote compatibility")
...
# FIX 3: Monkeypatch torch.load — only inject weights_only=False when caller did NOT
# specify it. This preserves any explicit weights_only=True from security-conscious
# third-party libraries, while still loading legacy checkpoints that pre-date the flag.
...
def main():
    ...
    print(f"  [DEBUG] HF model id: {HF_MODEL_ID}")
    ...
    wav, sr = _safe_audio_load(WAV_PATH)
    if wav.shape[0] > 1:
        wav = wav.mean(dim=0, keepdim=True)
    if sr != TARGET_SR:
        wav = torchaudio.functional.resample(wav, sr, TARGET_SR)
