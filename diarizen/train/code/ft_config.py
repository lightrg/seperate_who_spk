...
def build_phase_configs_test ()->List [PhaseConfig ]:
    ...
    lr =5e-5 ,
    warmup_steps =20 ,
    freeze_strategy ="top_N",
    unfreeze_top_n =4 ,
    data_mix =PhaseDataMix .fixed (
    {SOURCE_SELF :0.90 ,SOURCE_TIER_B :0.10 },n_epochs =2
    ),
    use_augmentation =False ,
    grad_clip =0.5 ,
    weight_decay =0.01 ,
    overlap_loss_weight =1.4 ,
    triple_loss_weight =1.8 ,
    ...
    unfreeze_top_n =6 ,
    data_mix =PhaseDataMix .fixed (
    {SOURCE_SELF :0.75 ,SOURCE_TIER_B :0.20 ,SOURCE_TIER_C :0.05 },
    n_epochs =1 ,
    ),
    use_augmentation =False ,
    grad_clip =0.5 ,
    weight_decay =0.01 ,
    overlap_loss_weight =1.7 ,
    triple_loss_weight =2.0 ,
    silence_loss_weight =1.0 ,
    ...
    unfreeze_top_n =6 ,
    data_mix =PhaseDataMix .fixed (
    {SOURCE_SELF :0.65 ,SOURCE_TIER_B :0.25 ,SOURCE_TIER_C :0.10 },
    n_epochs =1 ,
    ),
    use_augmentation =False ,
    grad_clip =0.5 ,
...
class FinetuneConfig :
    pretrained_model :str ="BUT-FIT/diarizen-wavlm-large-s80-md-v2"
    output_dir :str =str (
    _REPO_ROOT
    /"checkpoints"
    /"training"
    /("smoke_test"if TEST_MODE else "diarizen_vi_cuda_mdv2_multilabel_v1")
    )

    sample_rate :int =16000

    chunk_sec :float =16.0
    max_speakers :int =4

    batch_size :int =8 if TEST_MODE else 6
    grad_accum :int =1 if TEST_MODE else 2
    num_workers :int =0 if TEST_MODE else 8
    seed :int =42
    ...
    local_der_threshold :float =0.50
    local_der_threshold_grid :List [float ]=field (
    default_factory =lambda :[0.35 ,0.45 ,0.50 ,0.55 ]
    )
    ...
    dataloader_prefetch_factor :int =2 if TEST_MODE else 4
