...
MIN_RTTM_DUR_SEC      = 0.2          # bỏ RTTM segment quá ngắn
...
def scan_rttm_only(
    ...
    rttm_override: Optional[Path] = None,
) -> List[dict]:
    """
    ...
    # Handle case where split_root is already a specific case folder (e.g., data19)
    if split_root.is_dir() and (split_root / "labeled").exists() and (split_root / "mixture.wav").exists():
        data_dirs = [split_root]
    else:
        data_dirs = sorted(split_root.glob("data*"))
    ...
    for data_dir in data_dirs:
        ...
        if rttm_override and rttm_override.exists():
            rttm_path = rttm_override
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run ASR inference on RTTM segments. Does NOT use STM."
    )
    parser.add_argument("--dir",           required=True, help="Directory containing data* (with mixture.wav + labeled/*.rttm)")
    parser.add_argument("--rttm",          required=False, help="Explicit path to RTTM file. Overrides reading from labeled_dir.")
    parser.add_argument("--model",         required=True, help="Path to Whisper model / adapter")
    parser.add_argument("--out_csv",       required=True, help="Path to output CSV for predictions")
    parser.add_argument("--skip_unknown",  action="store_true", help="Skip segments labeled as speaker unknown_*")
    parser.add_argument("--trim_sec",      type=float, default=INFERENCE_TRIM_SEC,
                        help=f"Trim both ends of segments during inference (seconds). Default={INFERENCE_TRIM_SEC:.2f}s")
    ...
    rttm_override_path = Path(args.rttm) if args.rttm else None
    rows = scan_rttm_only(
        split_root,
        skip_unknown_speaker=args.skip_unknown,
        rttm_override=rttm_override_path,
    )
    if not rows:
